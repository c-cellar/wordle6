...
const init = () => {
  ...
  addEventhandlerDarkButtonDropdown();
  addEventhandlerLightButtonDropdown();
};

// === EVENTS & XHR =======
const addKeyhandler = () =>
  document.addEventListener("keydown", checksGetsLetterFromInput);
const addClickEventOnKey = () =>
  allKeys.forEach((key) =>
    key.addEventListener("click", checksGetsLetterFromInput)
  );
const addBtnHandlerNewGame = () =>
  btnNewGame.forEach((btn) => btn.addEventListener("click", newGame));
const addBtnHandlerShowWord = () =>
  btnShowWord.addEventListener("click", showWordInUnsuccessfulNotification);
const addEventHandlerMenu = () =>
  hamburgerMenuIcon.addEventListener("click", openMenu);
const addEventhandlerDarkBtn = () =>
  darkButtonRadio.addEventListener("click", changeThemeToDark);
const addEventhandlerLightBtn = () =>
  lightButtonRadio.addEventListener("click", changeThemeToLight);
const addEventhandlerLightButtonDropdown = () =>
  lightButtonDropdown.addEventListener("click", changeThemeToLight);
const addEventhandlerDarkButtonDropdown = () =>
  darkButtonDropdown.addEventListener("click", changeThemeToDark);
...
function changeThemeToLight() {
  ...
  // save the selected theme in local storage (kind of "default")
  setLocalStorageTheme();
}

// TODO: refactoring
// changes the theme to the dark-mode
...
const newGame = (selector) => {
  ...
  if (wasWordRevealed() || winningRound()) {
    ...
    setLocalStorageArrayWords(arrayWords);
  ...
  $("#searchedWord").style.display === "block"
    ? hideWordInUnsuccessfulNotification()
    : "";
};

// überprüft ob das Wort am Ende des Spiels vom Spieler aufgelöst wurde oder nicht
const wasWordRevealed = () =>
  $("#searchedWord").style.display === "block" ? true : false;

// Überprüft ob das Spiel gewonnen wurde oder nicht
const winningRound = () =>
  successfulNotification.style.display === "block" ? true : false;
...
const removeLetter = (arrayGuess) =>
  (idRoundSelector(counterRound)[arrayGuess.length].innerHTML = "");

// nächste Runde
const round = (counterRound) => {
  let roundSelector = $(
    ".letterContainer > div:nth-child(" + counterRound + ")"
  ).id;
